...
def get_needle_img(src):
    ...
    fill_needle_img = get_contours(denoise_img, 1000)
    ...
    return blood_roi, needle_theta,row,col


def get_vein_img(src,row,col):
    # step 6：识别血管及方向
    # step6-1:获取肘部区域
...
if __name__ == "__main__":
    ...
    # while 1:
    #     src = get_nir_img(cap)
    #     blood_roi, needle_theta,row,col = get_needle_img(src)
    #     thin_blood_img,blood_theta = get_vein_img(blood_roi,0,col)
    ...
    src = cv2.imread("F:/2projectInfo/blood_robot/original/needle4.png")
    blood_roi, needle_theta, row, col = get_needle_img(src)
    thin_blood_img, blood_theta = get_vein_img(blood_roi, 0, 150)
    cv2.imshow("blood_img", thin_blood_img)
    cv2.waitKey()

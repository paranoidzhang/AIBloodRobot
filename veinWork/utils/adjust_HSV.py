...
# img_original = cv2.imread("F:/2projectInfo/blood_robot/original/needle2.png")
# shapes = img_original.shape
# img_original = cv2.resize(img_original, (shapes[1] // 2, shapes[0] // 2))

cap = cv2.VideoCapture(1)  # 调整参数实现读取视频或调用摄像头
while 1:
    ret, img_original = cap.read()
    img_original = img_original[62:420, :]
    cv2.imshow("cap", img_original)
    # 颜色空间的转换
    ...
    cv2.createTrackbar('LowerbS', winName, 115, 255, nothing)
    cv2.createTrackbar('LowerbV', winName, 220, 255, nothing)
